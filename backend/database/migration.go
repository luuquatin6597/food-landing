...
// CreateTables tạo các bảng cần thiết trong database
...
// SeedData chèn dữ liệu mẫu vào database
func SeedData(db *sql.DB) error {
	...
	if count == 0 {
        ...
        ('Mì Quảng', 'Mì Quảng là món ăn đặc sản nổi tiếng nhất của Quảng Nam với nước dùng đậm đà từ tôm cua, thịt heo, trứng cút và bánh tráng nướng thơm ngon.', 'Mì vàng, Tôm tươi, Thịt heo, Trứng cút, Bánh tráng nướng, Rau thơm', '35.000 - 50.000 VNĐ', 'https://images2.thanhnien.vn/528068263637045248/2024/11/12/1000022316-1731386167793124667085.jpg', '#ff6b35', 'Quảng Nam'),
        ...
        ('Cơm Gà Tam Kỳ', 'Cơm Gà Tam Kỳ là món ăn đặc sản của Quảng Nam, với cơm gà thơm ngon, nước dùng đậm đà và rau sống.', 'Cơm, Gà, Nước dùng, Rau sống', '40.000 - 60.000 VNĐ', 'https://beptruong.edu.vn/wp-content/uploads/2020/02/com-ga-tam-ky.jpg', '#ff6347', 'Quảng Nam')
        `
...
// MigrateAndSeed thực hiện cả migration và seeding
func MigrateAndSeed(db *sql.DB) error {
	// Tạo bảng
	if err := CreateTables(db); err != nil {
		log.Printf("Không thể tạo bảng: %v", err)
		return err
	}

	// Chèn dữ liệu mẫu
	if err := SeedData(db); err != nil {
		log.Printf("Không thể chèn dữ liệu: %v", err)
		return err
	}
